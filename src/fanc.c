...
//    Time-stamp: <H. Nagata, 2013-11-21T18:39:33 JST>
// ---------------------------------------------------------
...
struct ctrl_fanc_t {
	...
	double *init_coef, max_rho, max_gamma, min_gamma, gamma_ebic;
	int num_rhos, num_gammas;
	int maxit_EM, maxit_CD, maxit_Phi;
	int cor_factor, use_penalty, zero_min_rho;
	int cold_init, num_iter_init, p_max_for_S;
	int progress, omp_level, num_threads;
};
...
static void pswap(void **p1, void **p2) {
	// "tmp" should not be static, but if so p2 will be still p2 in some cases
	// (and only on Linux). Why doesn't it work?
...
double MC(double theta, double rho, double gamma){
	double ans;
	if(rho==0.0) ans = 0.0;
	else if(fabs(theta) < rho*gamma) ans = rho*( fabs(theta) - theta*theta/(2.0*rho*gamma) );
	else  ans = rho*rho*gamma/2.0;
	return(ans);
}

double MC_onedimension(double theta, double theta_tilde, double rho, double gamma){
	double ans;
	ans = 1.0 / 2.0 * (theta - theta_tilde) * (theta - theta_tilde) + MC(theta, rho, gamma);
	return(ans);
}
...
double S_MC(double theta, double rho, double gamma)
{
	...
	if(gamma>1.0){
		if(fabs(theta) > rho*gamma) ans = theta;
		else if(fabs(theta) <= rho) ans = 0.0;
		else ans = delta;
	}else if(gamma<1.0){
        ...
        double kouho3 = MC_onedimension(signtheta*rho*gamma, theta, rho, gamma);
        if(kouho1 < kouho2 && kouho1 < kouho3){
            ans = theta;
        }else if(kouho2 < kouho3){
            ans = 0.0;
        }else{
            ans = signtheta*rho*gamma;
        }
	}else{
		ans=0.0;
	}
                            ...
                            double *O_Lambda_new, int *O_conv)
{
	...
	while (err_CD > ctrl->tol_CD && iter_CD < ctrl->maxit_CD) {
		for (int j = 0; j < m2; j++) {
            ...
            /*
             double delta = dsignum(z) * (fabs(z) - wrho) / (1 - 1 / wgamma);
             if (wgamma > 1) {
             if (fabs(z) > wrho * wgamma) {
             Lambda_i_new[j] = z;
             } else if (fabs(z) <= wrho) {
             Lambda_i_new[j] = 0;
             } else {
             Lambda_i_new[j] = delta;
             }
             } else if (wgamma < 1) {
             warning("gamma* was smaller than 1.0");
             if (fabs(z) > wrho * wgamma) {
             Lambda_i_new[j] = z;
             } else if (fabs(delta) <= wrho * wgamma / 2) {
             Lambda_i_new[j] = 0;
             } else {
             Lambda_i_new[j] = dsignum(z) * wrho * wgamma;
             }
             }
             */
		}
		...
		// Step
		pswap((void **)&Lambda_i, (void **)&Lambda_i_new);
		iter_CD++;
	}
...
static void calculate_logF(
                           ...
                           double *S, double *X, double *Im, double rho, double gamma,
                           int cor_factor, struct ctrl_fanc_t *ctrl,
...
if (ctrl->omp_level == 1) num_threads(ctrl->num_threads)
#endif
			for (int i = 0; i < p; i++) {
				update_Lambda_i(
		...
		for (int j = 0; j < m2; j++) {
			...
			if (sum_abs_Lambda_j > ctrl->tol_EM) {
				if (j2 < j) {
					F77_CALL(dcopy)(
		...
		// Step
		pswap((void **)&Lambda, (void **)&Lambda_new);
		pswap((void **)&diag_Psi, (void **)&diag_Psi_new);
		pswap((void **)&Phi, (void **)&Phi_new);
		iter_EM++;
	...
	calculate_logF(
                   p, m, N, Lambda, diag_Psi, Phi, S, X, Im, rho, gamma,
...
static void init_params_warm(int p, int m, int N,
                             double *S, double *X, double *Im, struct ctrl_fanc_t *ctrl,
                             double *O_Lambda, double *O_diag_Psi, double *O_rho, int *conv)
{
	void *vmax = vmaxget();
	int m2;
	...
	for (int t = 0; t < ctrl->num_iter_init; t++) {
		...
		minimize_logF_Lambda_Psi_Phi(p, 1, N, 1, Lambda_tmp, diag_Psi_tmp, Phi,
                                     S, X, Im, 0, R_PosInf, 0, ctrl,
                                     &m2, Lambda_tmp, diag_Psi_tmp, Phi,
                                     &logF_tmp, conv);
        
		// Update if better
		if (logF_tmp.logF_pen < min_logF_pen) {
			pswap((void **)&Lambda_tmp, (void **)&Lambda_opt);
			pswap((void **)&diag_Psi_tmp, (void **)&diag_Psi_opt);
	...
	pswap((void **)&diag_Psi_opt, (void **)&diag_Psi_init);
...
static void init_params_random(
                               ...
                               double *S, double *X, double *Im,
                               double rho, double gamma, int use_cor_fact, struct ctrl_fanc_t *ctrl,
                               int *O_m2, double *O_Lambda, double *O_diag_Psi, double *O_Phi, int *O_conv)
{
	...
	for (int t = 0; t < ctrl->num_iter_init; t++) {
		...
		if (logF_tmp.logF_pen < min_logF_pen) {
			...
			pswap((void **)&Lambda_tmp, (void **)&Lambda_opt);
			pswap((void **)&diag_Psi_tmp, (void **)&diag_Psi_opt);
			pswap((void **)&Phi_tmp, (void **)&Phi_opt);
			min_logF_pen = logF_tmp.logF_pen;
		}
	}
                 ...
                 int *O_conv)
{
	...
	double max_rho = ctrl->max_rho;
	if (ISNAN(max_rho) || !ctrl->cold_init) {
		init_params_warm(
                         p, m, N, S, X, Im, ctrl, Lambda_init, diag_Psi_init,
                         ISNAN(max_rho) ? &max_rho : NULL, O_conv);
	}
	if (max_rho > LARGE_RHO) {
		warning(
	...
	if (!ctrl->cold_init) {
		...
		F77_CALL(dcopy)(&pm, Lambda_init, INC_SEQ, Lambda, INC_SEQ);
		F77_CALL(dcopy)(&p, diag_Psi_init, INC_SEQ, diag_Psi, INC_SEQ);
		F77_CALL(dcopy)(&mm, Im, INC_SEQ, Phi, INC_SEQ);
	}
    
	// Determine rho and gamma
	gammas[0] = R_PosInf;
	logseq(num_gammas - 1, ctrl->max_gamma, ctrl->min_gamma, gammas + 1);
	logseq(num_rhos, max_rho, max_rho * ctrl->delta, rhos);
	for (int gi = 1; gi < num_gammas; gi++) {
		for (int ri = 0; ri < num_rhos; ri++) {
			rhos[ri + gi * num_rhos] = zeroin(
                                              rhos[ri], dmax(max_rho, 8.3), zeroinfn_rho, DBL_EPSILON,
                                              &(struct ex_zeroin_rho_t){gammas[gi], rhos[ri]});
		}
	}
	...
	for (int rgi = 0; rgi < num_gammas * num_rhos; rgi++) {
		...
		if (ctrl->cold_init) {
			init_params_random(
                               p, m, N, m, S, X, Im,
                               rhos[rgi], gammas[gi], ctrl->cor_factor, ctrl,
		...
		minimize_logF_Lambda_Psi_Phi(
                                     ...
                                     S, X, Im, rhos[rgi], gammas[gi], ctrl->cor_factor, ctrl,
		...
		if (gi == 0 && m2 < m) {
			...
			init_params_random(
                               p, m, N, m, S, X, Im, rhos[rgi], gammas[gi], 0, ctrl,
                               &m2_full, Lambda_full, diag_Psi_full, Phi_full, conv);
			minimize_logF_Lambda_Psi_Phi(
                                         p, m, N, m, Lambda_full, diag_Psi_full, Phi_full,
                                         S, X, Im, rhos[rgi], gammas[gi], 0, ctrl,
                                         &m2_full, Lambda_full, diag_Psi_full, Phi_full,
                                         &logF_full, conv);
            
			// Calculate precious value if the full model seems better
			if (ctrl->cor_factor &&
				m2_full > m2 && logF_full.logF_pen < logF.logF_pen)
			{
				minimize_logF_Lambda_Psi_Phi(
                                             p, m, N, m, Lambda_full, diag_Psi_full, Phi_full,
                                             S, X, Im, rhos[rgi], gammas[gi], 1, ctrl,
                                             &m2_full, Lambda_full, diag_Psi_full, Phi_full,
                                             &logF_full, conv);
			}
			...
			if (logF_full.logF_pen < logF.logF_pen) {
				m2 = m2_full;
				pswap((void **)&Lambda, (void **)&Lambda_full);
				pswap((void **)&diag_Psi, (void **)&diag_Psi_full);
				pswap((void **)&Phi, (void **)&Phi_full);
				logF = logF_full;
			}
		}
		...
		memcpy(O_conv + rgi * 3, conv, sizeof(int) * 3);
        
        //define df
        int c = ctrl->cor_factor ? m2 * (m2 - 1) / 2 : 0;
		O_df[rgi] = df_base[ri] + c;
        O_dfnonzero[rgi] = nonzero + p + c;
		...
		if (p <= ctrl->p_max_for_S) {
            // df
            double df = O_df[rgi];
            double df_nonzero = O_dfnonzero[rgi];
            
			//  Calculate GFI
			// Sigma <- Lambda %*% Phi %*% t(Lambda) + Psi
			double *LambdaPhi = (double *)R_alloc(pm, sizeof(double));
			...
			F77_CALL(dgemm)(
							...
							&D_ONE, LambdaPhi, &p, Lambda, &p, &D_ZERO, Sigma, &p);
			for (int i = 0; i < p; i++) {
				Sigma[i + i * p] += diag_Psi[i];
			}
			...
			for (int i = 0; i < p; i++) {
				for (int j = i; j < p; j++) {
					srmr += 2.0 / (p * (p + 1)) * (S[i + j * p] - Sigma[i + j * p]) * (S[i + j * p] - Sigma[i + j * p]) / (S[i + i * p] * S[j + j * p]);
				}
			}
			O_SRMR[rgi] = sqrt(srmr);
			
			// Calculate RMSEA
			double fhat = logF.logF - logdet_S - p;
			double rmsea2 = fhat/df - 1.0 / N;
			double rmsea2_dfnonzero = fhat/df_nonzero - 1.0 / N;
			if(rmsea2 < 0.0) O_RMSEA[rgi] = 0.0;
			if(rmsea2 >= 0.0)  O_RMSEA[rgi] = sqrt(rmsea2);
			if(rmsea2_dfnonzero < 0.0) O_RMSEA_dfnonzero[rgi] = 0.0;
			if(rmsea2_dfnonzero >= 0.0)  O_RMSEA_dfnonzero[rgi] = sqrt(rmsea2_dfnonzero);
			
			// Calculate CFI
			double df0 = p * ( p - 1.0 ) / 2.0;
			double CFI_n = fhat - df / N;
			double CFI_n_dfnonzero = fhat - df_nonzero / N;
			double CFI_d = f0hat - df0 / N;
			if(CFI_d < 0) CFI_d = 0.0;
			if(CFI_d < CFI_n) CFI_n = CFI_d;
			if(CFI_n < 0) CFI_n = 0.0;
			O_CFI[rgi] = 1 - CFI_n / CFI_d;
            
			if(CFI_d < CFI_n_dfnonzero) CFI_n_dfnonzero = CFI_d;
			if(CFI_n_dfnonzero < 0) CFI_n_dfnonzero = 0.0;
			O_CFI_dfnonzero[rgi] = 1 - CFI_n_dfnonzero / CFI_d;
		}
	}
...
if (ctrl->omp_level == 1) num_threads(ctrl->num_threads)
	...
	for (int rgi = 0; rgi < num_gammas * num_rhos; rgi++) {
		...
		int m2 = O_m2[rgi];
        int c = ctrl->cor_factor ? m2 * (m2 - 1) / 2 : 0;
		double logF = O_logF[0 + rgi * 3];
        
		// df
		double df = O_df[rgi];
        double nonzero = O_spn_Lambda[rgi];
		double df_nonzero = O_dfnonzero[rgi];
        
		// Criterion
		O_BIC[rgi] = N * logF + log(N) * df;
		O_BIC_dfnonzero[rgi] = N * logF + log(N) * df_nonzero;
		O_BICH[rgi] = N * logF + df * log(N) * log(log(p * m2 + p + c));
		O_EBIC[rgi] =
        O_BIC[rgi] + 2 * ctrl->gamma_ebic * (df - p - c) * log(p * m);
		O_EBIC_dfnonzero[rgi] =
        O_BIC_dfnonzero[rgi] + 2 * ctrl->gamma_ebic * (df_nonzero - p - c) * log(p * m);
		O_AIC[rgi] = N * logF + 2 * df;
		O_AIC_dfnonzero[rgi] = N * logF + 2 * df_nonzero;
		O_CAIC[rgi] = N * logF + (log(N) + 1) * df;
		O_CAIC_dfnonzero[rgi] = N * logF + (log(N) + 1) * df_nonzero;
		O_AGFI[rgi] =
        1 - (p * (p + 1) * (1 - O_GFI[rgi])) / (p * (p + 1) - 2 * df);
		O_AGFI_dfnonzero[rgi] =
        1 - (p * (p + 1) * (1 - O_GFI[rgi])) / (p * (p + 1) - 2 * df_nonzero);
	}
...
SEXP RextCall_fanc(
                   SEXP R_p, SEXP R_m, SEXP R_N, SEXP R_S, SEXP R_X, SEXP R_ctrl)
{
	__REXTCALL_INIT__
	GetRNGstate();
	...
	struct ctrl_fanc_t ctrl = {
		...
		REAL(get_list_element(R_ctrl, "rho.max"))[0],
	...
	REXTCALL_DEF(R_conv, allocVector(INTSXP, 3 * num_rhos * num_gammas));
	...
	pfill(num_rhos * num_gammas, (void **)spi_Lambda, NULL);
